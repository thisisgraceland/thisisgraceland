...
JSFX.fadeUp = function(elem, step)
{
	...
	if(elem)
	{
		...
		if(elem.fade2 == null)
		{
			elem.fade2 = new Object();
			elem.fade2.state	 = "OFF";
			elem.fade2.upStep	 = step;
			elem.fade2.downStep  = step;
			elem.fade2.minOpacity  = JSFX.FadeElemMinOpacity;
			elem.fade2.index = elem.fade2.minOpacity;
			elem.animate=JSFX.FadeElemAnimation;
			elem.fadeId="JSFX_FadeElem" + JSFX.FadeElemIdIndex++;
			window[elem.fadeId] = elem;

			if(elem.filters)
				elem.fade2.minOpacity = elem.filters.alpha.opacity;
			else if(document.defaultView.getComputedStyle(elem,'').getPropertyValue('-moz-opacity'))
				elem.fade2.minOpacity = document.defaultView.getComputedStyle(elem,'').getPropertyValue('-moz-opacity') * 100;
			
			
		}
		if(elem.fade2.state == "OFF")
		{
			elem.fade2.upStep  = step;
			elem.fade2.state = "FADE_UP";
			elem.animate();
		}
		else if( elem.fade2.state == "FADE_UP_DOWN"
			|| elem.fade2.state == "FADE_DOWN")
		{
			elem.fade2.upStep  = step;
			elem.fade2.state = "FADE_UP";
		}
...
JSFX.fadeDown = function(elem, step)
{
	if(elem)
	{
		...
		if(elem.fade2.state=="ON")
		{
			elem.fade2.downStep  = step;
			elem.fade2.state="FADE_DOWN";
			elem.animate();
		}
		else if(elem.fade2.state == "FADE_UP")
		{
			elem.fade2.downStep  = step;
			elem.fade2.state="FADE_UP_DOWN";
		}
	}
}
/*******************************************************************
...
JSFX.FadeImageAnimation = function()
{
	...
	for(i=0 ; i<document.elems.length ; i++)
	{
		...
		if(elem.fade2)
		{
			if(elem.fade2.state == "FADE_UP")
			{
				elem.fade2.index+=elem.fade2.upStep;
				if(elem.fade2.index > 100)
					elem.fade2.index = 100;

				if(elem.filters)
					elem.filters.alpha.opacity = elem.fade2.index;
				else
					elem.style.MozOpacity = elem.fade2.index/101;
				elem.style.opacity = elem.fade2.index/101;
			...
			else if(elem.fade2.state == "FADE_DOWN")
			{
				elem.fade2.index-=elem.fade2.downStep;
				if(elem.fade2.index < elem.fade2.minOpacity)
					elem.fade2.index = elem.fade2.minOpacity;
